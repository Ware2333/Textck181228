...
<body>
  ...
  <div class="layui-btn-container">
    <button class="layui-btn layui-btn-normal" lay-event="add">添加新信息</button>
    <div class="layui-inline"><div class="layui-input-inline"><input class = "layui-input layui-input-inline" type="text" id = "cust_code" placeholder = "请输入客户编号" style = "bottom: 5px; left : 5px"></div></div>
		...
		function init(user_code, cust_code) {
			table
					.render({
						...
						toolbar : '#toolbarDemo',
						totalRow : true,
						url : '/Textck181228/customer_communicationController/select.do' //数据接口
						,
						where : {
							cust_code : cust_code
						},
			...
			table.on('toolbar(test)',
					function(obj) {
						...
						case 'add':
							openLayer(
									'/jump/AddCustomer_Communication_Info.do',
									init2);
							break;
						case 'select':
							init("${code}", $("#cust_code").val());
							break;
						}
						;
					});
			table.on('tool(test)', function(obj) { //注：edit是固定事件名，test是table原始容器的属性 lay-filter="对应的值"
				if (obj.event == 'del') {
					ajax("/customer_communicationController/delete.do", {
						id : obj.data.id
					}, "text", function(data) {
			...
			table.on('edit(test)', function(obj) { //注：edit是固定事件名，test是table原始容器的属性 lay-filter="对应的值"
				...
				json["id"] = obj.data.id;
				ajax("/customer_communicationController/update.do", json, "text",
						function(data) {
							if (data == "success") {
								layer.msg("已修改");
								init("", "")
